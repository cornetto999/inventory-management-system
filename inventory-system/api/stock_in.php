...
if (request_method() === 'GET') {
    ...
    $params = [];
    if ($dateFrom !== '') {
        $where[] = 'si.created_at >= :date_from';
        $params[':date_from'] = $dateFrom . ' 00:00:00';
    }
    if ($dateTo !== '') {
        $where[] = 'si.created_at <= :date_to';
        $params[':date_to'] = $dateTo . ' 23:59:59';
    }
    $whereSql = $where ? (' WHERE ' . implode(' AND ', $where)) : '';

    $countStmt = $pdo->prepare('SELECT COUNT(*) FROM stock_in si' . $whereSql);
    $countStmt->execute($params);
    $total = (int)$countStmt->fetchColumn();

    $stmt = $pdo->prepare("\
        SELECT si.id, si.product_id, si.qty, si.cost_per_unit, si.supplier_id, si.remarks, si.created_at,\
    ...
    $items = array_map(function ($r) {
        return [
            ...
            'suppliers' => $r['supplier_name'] !== null ? ['name' => (string)$r['supplier_name']] : null,
        ];
    }, $rows);
...
if (request_method() === 'POST') {
    ...
    if ($costPerUnit < 0) api_error('Cost must be >= 0');
    ...
    try {
        ...
        $id = uuid_v4();
        $stmt = $pdo->prepare('INSERT INTO stock_in (id, product_id, qty, cost_per_unit, supplier_id, remarks, created_by) VALUES (:id, :product_id, :qty, :cpu, :supplier_id, :remarks, :created_by)');
        $stmt->execute([
            ':id' => $id,
        ...
        $pdo->rollBack();
        api_error('Failed to record stock in');
    }
}
